...
do
	...
	do
		echo "${names[i]}"
		time python scripts/train.py --pretrained-lm "${HOME_DIR}/continued_pretraining_directory_pubmedbert_10_epochs/" --num-train-epochs 10 --lr 2e-4 --batch-size 48 --training-file data/final_train_set.jsonl --test-file data/final_test_set.jsonl --context-window-size ${arr[i]} --max-seq-length ${seqlen[i]} --label2idx data/label2idx.json --seed $seed --unfreezing-strategy final-bert-layer --model-name ${names[i]}_${seed}  |& tee ~/logs/${names[i]}_${seed}.log
